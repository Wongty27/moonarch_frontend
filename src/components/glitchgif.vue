<template>
    <div class="wrapper" :style="{ opacity: opac / 100, height: wrapperHeight }">
      <div class="container" :style="{height: wrapperHeight}">
        <h1 class="glitch" data-text="STRING">STRING</h1>
        <v-btn to="/customise" v-if="showButton">Build My PC Now</v-btn>
      </div>
    </div>
  </template>
